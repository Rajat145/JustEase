...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, email, password, role, phone, address, barRegistration } = req.body;

      const exists = await User.findOne({ email });
      if (exists) return res.status(400).json({ success: false, message: 'Email already registered.' });

      const user = await User.create({ name, email, password, role: role || 'user', phone, address, barRegistration });
